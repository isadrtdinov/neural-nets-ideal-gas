...
def transform_dataset(dataset, device, dataset_name):
...
def get_datasets(
    ...
    if samples_per_label is not None:
        ...
        # reindex the raw data and targets
        np.random.shuffle(chosen_idxs)
        raw_train.data    = raw_train.data[chosen_idxs]
        raw_train.targets = targets_np[chosen_idxs].tolist()

    train_set = transform_dataset(raw_train, device, dataset)
    test_set = transform_dataset(raw_test, device, dataset)
