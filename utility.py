...
def get_model_parameters(model, requires_grad=False):
    if not requires_grad:
        return parameters_to_vector([param.data.clone() for param in model.parameters()])
    else:
        return parameters_to_vector([param.data.clone() for param in model.parameters() if param.requires_grad])
...
def get_model_gradients(model, requires_grad=False):
    ...
    for param in model.parameters():
        if param.grad is not None and param.requires_grad:
            flattened_gradients.append(param.grad.clone().view(-1))
